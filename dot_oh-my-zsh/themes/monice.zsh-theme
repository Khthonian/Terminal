...
git_current_changes() {
  ...
  if [ -n "$cb" ]; then
    ...
    local additions=$(git diff --shortstat | grep -o '[0-9]* insertions' | awk '{sum += $1} END {print sum}')
    local deletions=$(git diff --shortstat | grep -o '[0-9]* deletions' | awk '{sum += $1} END {print sum}')
    local changes_icon="\uf126"
